...
void AStarSearch(int start, int goal)
{
	...
	if (start < 0 || goal < 0 || start >= map_width*map_height || goal >= map_width*map_height) {
		return;
	}
	...
	estimated_cost[start] = Heuristic(start, goal);
