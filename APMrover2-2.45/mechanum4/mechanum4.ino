...
void compas_nav()
{
  ...
  float goal_lat[2]={426781700,42677960};
  ...
  if(north>-150 && north<150 && west >-150 && west<150)
  {
    gps_waypoint++;
  }
...
if(gps_count==10)
  ...
  lat,lastLat, goal_lat[gps_waypoint],lon,lastLon,goal_lon[gps_waypoint],ToDeg(atan(west/north)),ToDeg(dir),ToDeg(rotation), dx,dy,dir,theta);
  }
  gps_count++;
  if(gps_count>10)
  {
...
void fakeCompass()
{
  ...
  if((dy == 0) && (dx != 0)) dir = PI/2*dx/abs(dx);
  if ((dx == 0) && (dy != 0)) {
  if (dy > 0 ) dir = 0;
  if (dy < 0 ) dir = -PI;
  }
