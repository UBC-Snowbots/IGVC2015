...
void move_pwm()
{
  uint16_t wheels[4]; 
  //rotate side forward
  if(rc[3].control_in < 100 || trust_gps==1)//gps
  {
...
void compas_nav()
{
  ...
  //float goal_lat[2]={426781629,42677944};
  //float goal_lon[2]={-831954749,-831955031};
  //float goal_lat[2]={426781700,426779600};
  //float goal_lon[2]={-831954900,-831954900};
 
  float goal_lat[2]={ 426781629, 426779440};
  float goal_lon[2]={-831954749,-831955031};
  
 // float goal_lat[2]={ 426779440, 426781629};
 ///float goal_lon[2]={-831955031, -831954749};
  ...
  //take reading, go forward, take reading
  
  //diference to rotation, rotate till facing
  
  //move forward to target
  

  float north=goal_lat[gps_waypoint]-lat;//+=north, -=south
  float east=goal_lon[gps_waypoint]-lon;// -=west, +=east

//At way point yet?
  if((north > -250 && north < 250) && (east > -250 && east < 250))
  {
    if(gps_waypoint < 2) 
    {
      gps_waypoint++;
    ...
    hal.console ->  printf("WAY POINT PASSED!");
  }
  ...
  if ((east >= 0) && (north > 0)) Gquadrant = 1;
  if ((east < 0) && (north >= 0)) Gquadrant = 2;
  if ((east <= 0) && (north < 0)) Gquadrant = 3;
  if ((east > 0) && (north <= 0 )) Gquadrant = 4;
  
   phi=atan(east/north);
   if(phi < 0) phi = phi*(-1);
   
  if (Gquadrant == 1) goal_heading = PI/2 - phi;
  else if (Gquadrant == 2) goal_heading = -PI/2 + phi;
  else if (Gquadrant == 3) goal_heading = -PI/2 - phi;
  else if (Gquadrant == 4) goal_heading = PI/2 + phi;
 ...
 //if(rotation>2 || rotation<(-2))
  //{
   ...
   else if (rotation == 0) gps_z = 0; 
   
   
     if(north>-400 && north<400 && east >-400 && east<400)
  {
     gps_y=110; 
    if(rotation>0)
    {
...
if(gps_count==10)
  {
 // hal.console->printf("lat: %f, lastLat: %f, glat: %f, lon: %f, lastLon: %f, glon: %f,Heading: %.2f, Real: %f, rotation %f, dx %f, dy %f, dir %f, theta %f\n",
 // lat,lastLat, goal_lat[gps_waypoint],lon,lastLon,goal_lon[gps_waypoint],ToDeg(atan(east/north)),ToDeg(dir),ToDeg(rotation), dx,dy,dir,theta);
 //hal.console -> printf(" lat %f, lon %f,goal_heading %f, north %f, west %f, phi %f, dir %f, rotation %f, gps_z %f, waypoint %d \n", lat, lon, ToDeg(goal_heading), north, east, ToDeg(phi), ToDeg(dir), ToDeg(rotation), gps_z, gps_waypoint);
 // lat,lastLat, goal_lat[gps_waypoint],lon,lastLon,goal_lon[gps_waypoint],ToDeg(atan(east/north)),ToDeg(dir),ToDeg(rotation), dx,dy,dir,theta);
  }
  gps_count++;
  if(gps_count>10)
...
void fakeCompass()
{
  ...
  if(lon-lastLon != 0)dx = lon - lastLon;
  if(lat-lastLat != 0)dy = lat - lastLat;
  int quadrant = 0;
  ...
  if (quadrant == 1) angle = PI/2 - theta;
  else if (quadrant == 2) angle = -PI/2 + theta;
  else if (quadrant == 3) angle = -PI/2 - theta;
  else if (quadrant == 4) angle = PI/2 + theta;
  
  lastLat = lat;
  lastLon = lon;
  
  
  if(dy != 0 && dx != 0) dir = angle;
  if((dy == 0) && (dx != 0)) 
  {
    if(dx>0) dir = +PI/2;
    if(dx<0) dir = -PI/2;
