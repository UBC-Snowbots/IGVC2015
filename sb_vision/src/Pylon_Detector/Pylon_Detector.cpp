...
int main(int argc, char **argv){
    // Stuff to run once, initializes your ROS node
...
void analyseStream( Mat inputImage ){
	...
	Mat inputImageModified = inputImage;
	...
	for( int i = 0; i < boundRect.size() ; i++ ){
		...
		for( int i = 0; i < boundRect.size(); i++ ){
			...
			determineIfRectRelated( inputImageModified, currRect, nextRect );
		}
	}
	...
	imshow("Bounding Box", inputImageModified);
...
vector<Rect> retrieveContours( Mat filteredImage ){
	...
	//// Debug purposes: draw bounding rects
