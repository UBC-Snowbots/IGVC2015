...
#include <opencv2/core/core.hpp>	// for matdata structure
#include <opencv2/stitching/stitcher.hpp>
#include "opencv2/highgui/highgui.hpp"	// for imread,imshow, waitkey, and namedwindow
#include <iostream> 
#include <vector>
...
static const int MSG_QUEUE_SIZE = 20;
 ...
 init(argc, argv, NODE_NAME); //initializes your ROS node
	...
	while (ros::ok())

		{
			...
			ROS_INFO("Starting Video Capture..."); // Publishes to log and terminal
			...
			//TODO: Google VideoCapture API, find a close() function or something that closes the connection
				...
				// get parameters
				//double cap1Width = cap1.get(CV_CAP_PROP_FRAME_WIDTH); //get the width of frames of the video
   				//double cap1Height = cap1.get(CV_CAP_PROP_FRAME_HEIGHT); //get the height of frames of the video
				

							
				 //create img frames
				// load caps into images
				Mat image1;
				...
				*/
				...
				if (!cap1.read(image1)){
				  ...
				  */
				  ...
				  // push_back append element to end of vector
				  // pop_back erases the last element of the vector
				...
				imgs.pop_back();
					...
					waitKey(500);	// wait half a second
			}
			return 0;
